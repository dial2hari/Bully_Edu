...
# Rounding the values of Prob_aggression
agg_df['Prob_aggression'].round(decimals=1)
att_df['Prob_attack'].round(decimals=1)
tox_df['Prob_toxicity'].round(decimals=1)

# Assigning data types for numbers for all the datasets
agg_df['Prob_aggression'] = agg_df['Prob_aggression'].astype(str)
att_df['Prob_attack'] = att_df['Prob_attack'].astype(str)
tox_df['Prob_toxicity'] = tox_df['Prob_toxicity'].astype(str)
...
def train_model(predictor,target):
    ...
    classifier.fit(predictor, target)
    return classifier

# Training the model
classifier_agg = train_model(X_train_agg_tf,y_train_agg)
classifier_att = train_model(X_train_att_tf,y_train_att)
classifier_tox = train_model(X_train_tox_tf,y_train_tox)


print(type(classifier_agg))
...
y_pred_agg = predictor(classifier_agg,X_test_agg_tf)
y_pred_att = predictor(classifier_att,X_test_att_tf)
y_pred_tox = predictor(classifier_tox,X_test_tox_tf)
...
score_agg = accuracy_score(y_test_agg, y_pred_agg_1)
print(score_agg)

score_att = accuracy_score(y_test_att, y_pred_att_1)
print(score_att)

score_tox = accuracy_score(y_test_tox, y_pred_tox_1)
print(score_tox)
...
agg = []
agg <- py_load_object(classifier_agg.pkl, pickle = "pickle")
classifier_att <-
...
pickle.dump(classifier_agg,open('classifier_agg.pkl',"wb"))
pickle.dump(classifier_att,open('classifier_att.pkl',"wb"))
pickle.dump(classifier_tox,open('classifier_tox.pkl',"wb"))
...
regressor_agg = pickle.load(open('classifier_agg.pkl',"rb"))
regressor_att = pickle.load(open('classifier_att.pkl',"rb"))
regressor_tox = pickle.load(open('classifier_tox.pkl',"rb"))

vectorizer_agg = pickle.load(open('classifier_agg.pkl',"rb"))
vectorizer_att = pickle.load(open('classifier_att.pkl',"rb"))
vectorizer_tox = pickle.load(open('classifier_tox.pkl',"rb"))

classifier_agg_file = "classifier_agg.joblib"
joblib.dump(classifier_agg, classifier_agg_file)

classifier_att_file = "classifier_att.joblib"
joblib.dump(classifier_att, classifier_att_file)

classifier_tox_file = "classifier_tox.joblib"
joblib.dump(classifier_tox, classifier_tox_file)
